...
export function WithdrawPiggyModal({visible,onClose,onSave,members,customCats=[],available=0}){
  ...
  return(
    <Modal visible={visible} onClose={onClose} title="🐷 Снять с копилки" onSave={save} saveLabel="Списать и потратить">
      <div style={{padding:16,paddingBottom:40}}>
        <div style={{...s.card,background:C.greenL,border:`.5px solid ${C.greenB}`,padding:'10px 12px',marginBottom:14,textAlign:'center'}}>
        ...
        <div style={{display:'flex',gap:7,overflowX:'auto',marginBottom:14,paddingBottom:4}}>
          {allCats.map(cat=><button key={cat.id} onClick={()=>setCatId(cat.id)} style={{display:'flex',alignItems:'center',gap:5,flexShrink:0,padding:'8px 11px',borderRadius:20,border:`.5px solid ${catId===cat.id?C.orangeB:C.border}`,background:catId===cat.id?C.orangeL:'#fff',color:catId===cat.id?'#991B1B':C.muted,fontSize:11,cursor:'pointer',fontFamily:'inherit'}}><span style={{fontSize:15}}>{cat.emoji}</span>{cat.name}</button>)}
        </div>
        <div style={{fontSize:10,color:C.muted,textTransform:'uppercase',letterSpacing:.5,marginBottom:6}}>Кто потратил</div>
        <div style={{display:'flex',gap:4,marginBottom:14}}>
          {members.map(m=><button key={m.id} onClick={()=>setMemberId(m.id)} style={{flex:1,padding:8,borderRadius:7,border:'none',background:memberId===m.id?C.orangeL:'#f1f5f9',color:memberId===m.id?'#991B1B':C.muted,fontSize:12,fontWeight:memberId===m.id?600:400,cursor:'pointer',fontFamily:'inherit'}}>{m.avatar} {m.name}</button>)}
        </div>
        <div style={{...s.card,background:C.yellowL,border:`.5px solid ${C.yellowB}`,padding:'9px 12px',marginBottom:16}}>
