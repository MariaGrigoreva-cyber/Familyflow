...
const s={
  card:{background:'#fff',borderRadius:14,border:`1px solid ${C.border}`,padding:14,marginBottom:8},
  ...
  input:{width:'100%',padding:12,borderRadius:12,border:`1px solid ${C.border}`,fontSize:14,color:C.text,background:'#fff',boxSizing:'border-box',fontFamily:'inherit',outline:'none'},
  ...
  return(
    <div style={s.modal} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={s.modalBox}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 16px',background:'#fff',borderBottom:`1px solid ${C.border}`,borderRadius:'16px 16px 0 0',flexShrink:0}}>
  ...
  <div style={{marginBottom:12}}>
    ...
    <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
      {Array.from({length:31},(_,i)=>i+1).map(d=>{const on=selected.includes(d);return(
        <button key={d} onClick={()=>onToggle(d)} style={{width:34,height:34,borderRadius:17,border:`1px solid ${on?C.orange:C.border}`,background:on?C.orange:'#fff',color:on?'#fff':C.text,fontFamily:MONO,fontSize:11,fontWeight:on?600:400,cursor:'pointer'}}>{d}</button>
      );})}
    </div>
  </div>
);
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:1,background:C.border,borderRadius:12,overflow:'hidden',marginBottom:14}}>
        {['1','2','3','4','5','6','7','8','9','000','0','⌫'].map(k=>(
          <button key={k} onClick={()=>press(k==='⌫'?'del':k)} style={{padding:'13px 0',background:'#fff',border:'none',fontFamily:MONO,fontSize:20,color:C.text,cursor:'pointer'}}>{k}</button>
        ))}
      </div>
    </div>
  );
    ...
    <div style={{padding:16,paddingBottom:32,display:'flex',flexWrap:'wrap',gap:8}}>
      {FACE_EMOJIS.map(e=>(
        <button key={e} onClick={()=>{onPick(e);onClose();}}
          style={{width:44,height:44,borderRadius:12,border:`1px solid ${selected===e?C.orange:C.border}`,background:selected===e?C.orangeL:'#fff',fontSize:22,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>{e}</button>
      ))}
    </div>
  </Modal>
);
