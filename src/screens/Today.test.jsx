...
test('показывает остаток на руках и советы', () => {
  render(<TodayScreen state={state} onToggle={noop} onAdd={noop} onEditPayment={noop} freeSpendableNow={freeSpendableNow} weeklyBalances={weeklyBalances} />);
  expect(screen.getByText('ОСТАТОК НА РУКАХ')).toBeInTheDocument();
  expect(screen.getByText('СОВЕТЫ')).toBeInTheDocument();
});

test('«+ Добавить запись» вызывает onAdd', async () => {
...
test('«Как это работает» открывает и закрывает слайды', async () => {
  const user = userEvent.setup();
  render(<TodayScreen state={state} onToggle={noop} onAdd={noop} onEditPayment={noop} freeSpendableNow={freeSpendableNow} weeklyBalances={weeklyBalances} />);
  await user.click(screen.getByText('Как это работает'));
  expect(screen.getByText('Система четырёх счетов')).toBeInTheDocument();
  await user.click(screen.getByText('← Назад'));
  expect(screen.queryByText('Система четырёх счетов')).not.toBeInTheDocument();
});

test('отметка предстоящего платежа вызывает onToggle', async () => {
  const user = userEvent.setup();
  const onToggle = jest.fn();
  render(<TodayScreen state={state} onToggle={onToggle} onAdd={noop} onEditPayment={noop} freeSpendableNow={freeSpendableNow} weeklyBalances={weeklyBalances} />);
