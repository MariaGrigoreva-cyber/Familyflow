...
export const logout = () => {
  ...
  // Кеш тарифа привязан к конкретному аккаунту — иначе следующий, кто войдёт на
  // этом устройстве, на секунду увидит чужой статус подписки (см. lib/plan.js).
  try { localStorage.removeItem('ff_billing_cache'); } catch {}
...
export const errText = e => ({
  ...
  // 402 от платных эндпоинтов. Отдельный код, а не pro_required — у того текст
  // про участников семьи.
  //
  // Текст намеренно НЕ говорит «данные сохранены, оформите подписку, чтобы
  // продолжить»: вести бюджет можно и на бесплатном тарифе, так что это
  // сообщение обещало бы потерю доступа, которой нет. 402 приходит только на
  // платные возможности, и почти всегда его перехватывает конкретный экран
  // (помощник показывает своё предложение) — сюда попадает лишь остаток.
  subscription_required: 'Это возможность подписки Pro — откройте «Настройки → Тариф»',
